...
// ── DB ──────────────────────────────────────────────────────────────
const DB_PATH = process.env.DB_PATH || path.join(__dirname, 'data', 'ctr.db');
const UPLOAD_DIR = process.env.UPLOAD_DIR || path.join(__dirname, 'public', 'uploads');

fs.mkdirSync(path.dirname(DB_PATH), { recursive: true });
fs.mkdirSync(UPLOAD_DIR, { recursive: true });
fs.mkdirSync(path.join(UPLOAD_DIR, 'foto'), { recursive: true });
fs.mkdirSync(path.join(UPLOAD_DIR, 'video'), { recursive: true });
...
// Seed admin note
const noteExists = db.prepare('SELECT id FROM admin_note WHERE id=1').get();
if (!noteExists) {
...
// Seed soci demo
const sociCount = db.prepare('SELECT COUNT(*) as n FROM soci').get();
if (sociCount.n === 0) {
  const ins = db.prepare(`INSERT INTO soci (nome,cognome,tessera,email,tel,nascita,cf,indirizzo,citta,cap,ruolo,stato,quota,iscritto) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
  const seed = [
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const type = req.params.type || 'foto';
...
app.get('/api/soci', (req, res) => {
  const rows = db.prepare('SELECT * FROM soci ORDER BY cognome').all();
  res.json(rows);
});

app.post('/api/soci', (req, res) => {
  const b = req.body;
  // Generate next tessera number
  const maxRow = db.prepare("SELECT tessera FROM soci WHERE tessera LIKE 'CTR-%' ORDER BY tessera DESC").get();
  const maxNum = maxRow ? parseInt(maxRow.tessera.replace('CTR-','')) : 0;
  const tessera = 'CTR-' + String(maxNum + 1).padStart(3, '0');
  try {
    const r = db.prepare(`INSERT INTO soci (nome,cognome,tessera,email,tel,nascita,cf,indirizzo,citta,cap,ruolo,stato,quota,note)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0,?)`).run(
      b.nome, b.cognome, tessera, b.email||'', b.tel||'',
...
app.patch('/api/soci/:id', (req, res) => {
  ...
  try {
    db.prepare(`UPDATE soci SET nome=?,cognome=?,email=?,tel=?,nascita=?,cf=?,indirizzo=?,citta=?,cap=?,ruolo=?,stato=?,note=? WHERE id=?`).run(
      ...
      b.nascita||null, b.cf||'', b.indirizzo||'', b.citta||'', b.cap||'',
      b.ruolo||'Socio', b.stato||'Attivo', b.note||'', id
    );
    res.json(db.prepare('SELECT * FROM soci WHERE id=?').get(id));
  } catch(e) { res.status(400).json({ error: e.message }); }
});
...
app.get('/api/quota2026', (req, res) => {
  res.json(db.prepare('SELECT * FROM quota2026').all());
});
...
app.post('/api/quota2026', (req, res) => {
  ...
  try {
    db.prepare(`INSERT INTO quota2026(socio_id,importo,data_pag,metodo,note,pagato)
      VALUES(?,?,?,?,?,1) ON CONFLICT(socio_id) DO UPDATE SET
...
app.get('/api/diano2026', (req, res) => {
  res.json(db.prepare('SELECT * FROM diano2026').all());
});
...
app.post('/api/diano2026', (req, res) => {
  ...
  try {
    db.prepare(`INSERT INTO diano2026(socio_id,acconto,saldo,data_iscr,metodo,camera,note)
      VALUES(?,?,?,?,?,?,?) ON CONFLICT(socio_id) DO UPDATE SET
      acconto=excluded.acconto, saldo=excluded.saldo, data_iscr=excluded.data_iscr,
      metodo=excluded.metodo, camera=excluded.camera, note=excluded.note`).run(
      b.socio_id, b.acconto||0, b.saldo||0, b.data_iscr, b.metodo, b.camera||'', b.note||''
    );
    res.json({ ok: true });
  } catch(e) { res.status(400).json({ error: e.message }); }
});
...
app.get('/api/gallery/:type', (req, res) => {
  const table = req.params.type === 'foto' ? 'gallery_foto' : 'gallery_video';
...
app.post('/api/gallery/:type', upload.single('file'), (req, res) => {
  ...
  try {
    if (type === 'foto') {
      ...
      const r = db.prepare('INSERT INTO gallery_video(titolo,mime_type,file_path,durata) VALUES(?,?,?,?)').run(titolo, req.file.mimetype, file_path, durata);
...
app.listen(PORT, () => console.log(`CTR La Röda → http://localhost:${PORT}`));
